        ...
        <>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', height: '100vh', width: '50vw', p: 4 }}>
                    ...
                    <h1 className="text-black font-bold px-4">Pilih Nominal Donasi</h1><br />
                    <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: '30px' }}>
                        {[5000, 10000, 25000, 50000, 100000, 150000].map(amount => (
                            <Card key={amount} onClick={() => {
                                setJumlah(amount);
                                setSelectedAmount(amount);
                            }}
                                className={`p-4 cursor-pointer mb-4 ${selectedAmount === amount ? 'selected-card' : ''}`}
                                style={{ width: '30%', backgroundColor: selectedAmount === amount ? 'lightblue' : '', textAlign: 'center', marginBottom: '20px' }}
                            >
                                <CardContent><span style={{ fontWeight: 'bold' }}>Rp.{amount}</span></CardContent>
                            </Card>
                        ))}
                    </div>
                    <br />
                    ...
                    <TextField type='number' label="Jumlah Rp." value={jumlah} onChange={(e) => setJumlah(e.target.value)} sx={{ mb: 2 }} /> <br />
